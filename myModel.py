...
class UnetSkipConnectionBlock(nn.Module):
    ...
    def __init__(self, before_exit = 3, before_enter = 3, in_channels=None,
                 ...
                 use_dropout=False, crop_weight = -0.99):
...
class UResGen(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, ngf=64, norm_layer=nn.BatchNorm2d, dropout_rate=0, n_blocks=6,
        ...
        for i in range(n_blocks):  # add ResNet blocks
            ...
            self.res_blocks+=[ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, \
        ...
        for i in range(n_downsampling):  # add upsampling layers
            mult = 2 ** (n_downsampling - i)
            self.up_layer.append(ResUNetUp(ngf * mult, int(ngf * mult / 2), dropout=0, \
                                           ad_res=ad_res[n_downsampling-i-1],ks=3))
    
        self.out_layer = nn.Sequential(*[nn.ReflectionPad2d(3),nn.Conv2d(ngf, out_channels, \
